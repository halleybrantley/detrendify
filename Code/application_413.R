...
spodFig <- data.frame(time = spodPIDs$time,
                      detrendr = spod_trends$e_0.05,
                      qsreg = qsreg_trends$e_0.05) %>%
...
ggplot(spodFig, aes(x=time, y=value)) +
  ...
  scale_color_manual(breaks = c("detrendr", "qsreg"),
                     values = c(colPal)) +
  labs(col="", x="", y="PID") +
xlim(c(spodPIDs$time[1], spodPIDs$time[15000])) 
  ylim(c(1, 2))
...
conf <- metric_df %>% 
  ...
  select(-metric_type) %>% unnest() %>% arrange(tau, crit)
conf
