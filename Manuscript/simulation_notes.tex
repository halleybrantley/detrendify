...
We compare the performance of our quantile trend filtering model with the models of 3 others using designs proposed by \cite{Racine2017}. The models compared are 
...
\begin{itemize}
	...
	\end{equation}
	The function \texttt{qsreg} in the texttt{fields} R package was used. 
	...
	$\sum_i\rho_{\tau}(y_i - \theta_i) + \lambda||D\theta||_1$ and $\lambda$ is chosen using SIC from above. 
...
Three simulation designs from \cite{Racine2017} were considered. For all designs $X_i$ was generated as a uniformly space sequence in $[0,1]$ and the mean function was given as 
$$Y_i = sin(2\pi X_i) + \epsilon_i(X_i)$$
The three error distributions considered were 
\begin{itemize}
 \item Gaussian: $\epsilon_i \sim N\left(0, \left(\frac{1+x_i^2}{4}\right)^2\right)$
...
100 datasets were generated of sizes 300, 500 and 1000. The MSE was calculated as $\frac{1}{n}\sum_i (\hat{q}_{\tau}(x_i) - q_\tau(x_i))^2$.	
