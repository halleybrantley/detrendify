...
%	 Given quantiles $\tau_1 < \cdots < \tau_J$, we define $\theta_{j,m}(t)$ as the value of the $\tau_j$\textsuperscript{th} quantile trend in window $m$ at time $t$. In order to constrain the overlapping sections to be equal, we define a consensus variable	 
	 Given quantiles $\tau_1 < \cdots < \tau_J$, we introduce dummy variables $\Vn{\theta}{m}_{j} \in \Real^{\lvert \mathcal{I}_m \rvert}$ as the value of the $\tau_j$\textsuperscript{th} quantile trend in window $m$. We then ``stitch" together the $M$ quantile trend estimates into consensus over the overlapping regions by introducing the constraint $\VnE{\theta}{m}{ij} = \VE{\theta}{i + l_m - 1,j}$ for $i = 1, \ldots, u_m - l_m + 1$ and for all $j$. Let $\Mn{\Theta}{m}$ be the matrix whose $j$th column is $\Vn{\theta}{m}$. We can write these constraints more compactly as $\Mn{\Theta}{m} = \Mn{U}{m}\M{\Theta}$, where $\Mn{U}{m} \in \{0, 1\}^{\lvert \mathcal{I}_m \rvert \times n}$ is a matrix that selects rows of $\M{\Theta}$ corresponding to the $m$th window. Furthermore, let $\iota_{\mathcal{C}}$ denote the indicator function of the non-crossing quantile constraint, namely $\iota_{\mathcal{C}}(\M{\Theta})$ is zero if $\M{\Theta} \in \mathcal{C}$ and infinity otherwise.
	 
	Our windowed quantile trend optimization problem can then be written as 
	 \begin{align*}
		 \label{eq:quantile_windows}
		 &\text{minimize}\; \sum_{m=1}^M \left \{\sum_{j=1}^J \left [\rho_{\tau_j}(\Vn{y}{m} - \Vn{\theta}{m}_{j}) + 
		 \lambda_j \lVert \Mn{D}{k+1} \Vn{\theta}{m}_{j} \rVert_1 \right ] + \iota_\mathcal{C}(\Mn{\Theta}{m}) \right\}\\
		 &\text{subject to} \qquad \Mn{\Theta}{m} = \Mn{U}{m}\M{\Theta} \;\; \text{ for } m = 1, \ldots, M.
	 \end{align*}	 
	 
	 	 Let $\Mn{\Omega}{m}$ denote the Lagrange multiplier matrix for the $m$th consensus constraint, namely $\Mn{\theta}{m} = \Mn{U}{m}\M{\theta}$, and let $\Vn{\omega}{m}_{j}$ denote its $j$th column.
%		 
%		  $\Vn{\omega}{m}_{j}$ denote Lagrange multipliers for the constraint $\Vn{\theta}{m}_j = \Mn{U}{m}\V{\theta}_j$, $\Mn{\Omega}{m}$ denote the matrix whose $j$th column is $\Vn{\omega}{m}_j$, and penalty parameter $\gamma > 0$, 
		  
		  The augmented Lagrangian is given by
	 \begin{eqnarray*}
	 \mathcal{L}(\M{\Theta}, \{\Mn{\Theta}{m}\}, \{\Mn{\Omega}{m}\}) & = & \sum_{m=1}^M \biggl \{\sum_{j=1}^J \biggl [\rho_{\tau_j}(\Vn{y}{m} - \Vn{\theta}{m}_j)+\lambda \lVert \Mn{D}{k+1}\Vn{\theta}{m}_j\rVert_1 \\
	 && + \; (\Vn{\theta}{m}_j - \Mn{U}{m}\V{\theta}_{j})\Tra\Vn{\omega}{m}_{j} + 
	 \frac{\gamma}{2}\lVert \Vn{\theta}{m}_j - \Mn{U}{m}\V{\theta}_{j}\rVert_2^2 \biggr ] + \iota_\mathcal{C}(\Mn{\Theta}{m}) \biggr\},
	 \end{eqnarray*}
...
%	\end{eqnarray*}
	\begin{eqnarray*}
	r_{\text{primal}}^{k} & = & \sqrt{\sum_{m=1}^M\lVert\Mn{\Theta}{m}_{k} - \M{\Theta}_k\rVert_{\text{F}}^2},
	\end{eqnarray*}
...
%	\end{eqnarray*}
	\begin{eqnarray*}
	r_{\text{dual}}^{k} & = & \gamma\sqrt{\sum_{m=1}^M \lVert\M{\Theta}_k - \M{\Theta}_{k-1}\rVert_{\text{F}}^2},
	\end{eqnarray*}
...
%	\end{eqnarray*}
	\begin{eqnarray*}
		r_{\text{primal}}^{k} & < &\epsilon_{\text{abs}}\sqrt{nJ} + \epsilon_{\text{rel}}\,\underset{m}{\max} \left[\max 
		\left\{\lVert\Mn{\Theta}{m}_k \rVert_{\text{F}}, \lVert \M{\Theta}_{k} \rVert_{\text{F}} \right\}\right] \\%\lVert \M{\Theta}_{k} \rVert_{\text{F}}} \right )\right]\\
		r_{\text{dual}}^{k} & < & \epsilon_{\text{abs}}\sqrt{nJ} + \epsilon_{\text{rel}}\,\sqrt{\sum_{m=1}^M
		\lVert \Mn{\Omega}{m}_k \rVert_{\text{F}}^2}.
	\end{eqnarray*}	
