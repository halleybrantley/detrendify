...
\begin{document}
	...
	\begin{algorithm}
		...
		\begin{algorithmic}
			\State{Given $y$ divided into overlapping windows $\{y_1, ..., y_M\}$ and quantiles $\{\tau_1 < ... < \tau_J\}$}
			\State {\textbf{initialize:} \\
				 $\theta_{j,m}^{(0)} = \arg \min \sum_{j=1}^J\rho_{\tau_j}(y_m - \theta_{j,m})+\lambda \lVert D\theta_{j,m}\rVert_1$ subject to $\theta_{1,m} < ...<\theta_{J,m}$. \\
				 $\bar{\theta}_{j,m}^{(0)}$ is the mean of $\theta_{j,m}^{(0)}$ and all overlapping windows.}\\
