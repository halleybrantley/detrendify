...
\V{x}^+ & = & \prox{t\psi}{\V{z}} \\
\V{y}^+ & = & P_V(2\V{x}^+ - \V{z}) \\
\V{z}^+ & = & \V{z} + \lambda(\V{y}^+ - \V{x}^+).
\end{eqnarray*}
The parameter $t$ is a step-size and $\lambda_n$ is Karolinski-Mann. We require that $\lambda_n \in ]0, 2[$ and that $\sum_n \lambda_n(1-\lambda_n) = \infty$. The mapping $P_V$ is the orthogonal projection onto the set $V$.
...
\item One could also apply Anderson acceleration to reduce the number of Spingarn updates, since the Douglas-Rachford algorithm is a fixed point algorithm.
\end{itemize}
