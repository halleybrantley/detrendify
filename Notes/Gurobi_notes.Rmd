...
Define $\hat{f}(x, \tau) = \sum_i \frac{1}{2*10^4\pi}log[cosh(10^4\pi x_i)] + (\tau - \frac{1}{2}) x_i$

min $\hat{f}(y - \theta, \tau) + \hat{f}(D\theta, 0.5)$

Both gradient and Hessian are easy to compute. Hessian is banded and sparse. 

## Compare times on n=200
```{r, cache = TRUE}
lambda <- 200/2
y <- spodNode$pid[1:200]
theta_g <- gurobi_trend(y, tau, 200, k)
D <- get_Dk(length(y), k)
...
knitr::kable(data.frame(method = c("Gurobi", "LBFGS", "Trust Region"), 
...
y <- df$pid[1:3600]
n <- length(y)
lambdaSeq <- c(10*n, 12*n, 14*n, 16*n)
