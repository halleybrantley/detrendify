...
for (i in 1:length(nSeq)){
  simData <- simulate(nSeq[i], k)
  lambda <- lambda0*nSeq[i]^k/factorial(k)
  y <- simData$y
  theta <- simData$y
  D <- simData$D
  eta <- matrix(D%*%theta)
  ...
  timing$R[i] <- system.time( 
      multi_step_R <- spingarn_multi_step_R(theta, eta, y, D, M, lambda, tau, step, 100))[3]
  timing$C[i] <- system.time(
    multi_step <- spingarn_multi_step(theta, eta, y, D, cholM,
                                     lambda, tau, step, 100, k))[3]
}


plot(C~n, timing, type="l", col="blue", ylab="Elapsed Time (s) per 100 iterations", main = "R code is in black, C in blue", ylim = c(0, 1))
